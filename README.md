...
The three `h5-url-*.html` files are synthetic heldout-v5 inputs. Their exact bytes are preregistered in `rekue/evaluation/heldout-v5/assets/register.jsonl`. The user approved the Source and browser-task composition only; no retained Source, Claim, or evidence has been approved as gold. Publication alone is not a URL capture, model judgement, human browser result, or quality score. Rekue will verify the public Pages bytes and import each page through its actual URL path after this change reaches the publishing branch.

The three `h7-url-*.html` files are synthetic heldout-v7 inputs. Their exact bytes are preregistered in `rekue/evaluation/heldout-v7/assets/register.jsonl`. The user approved only the Source and browser-task composition. Publication alone is not a retained URL Source, human gold, model judgement, browser result, or quality score. Rekue will verify the public Pages bytes and ingest through its actual URL route after this change reaches the publishing branch.
